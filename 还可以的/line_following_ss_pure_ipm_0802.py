...
0802-1834：还可以；
隐患：
1. 有小概率出现死区，会恒定偏离中线导致压内侧线
2. 整体偏内侧，与内侧线冗余较小
3. 需要第一步路口转向的时候尽可能对准车道，否则在只能认出来单边的情况下，有可能会把左右车道线认错，导致推测的中线飞错地方
...
TUNE = {
    'roi_bottom_ratio': 0.30,   # 检测 ROI：画面下方比例（越大看得越远，采样越足）
    ...
    'lookahead_px': 120.0,      # 前瞻距离 (px)，决定方位角=前瞻点横向偏/前瞻距离
    ...
    # ---------- PID 控制参数（非线性增益+分段，网页可调） ----------
    'kp_heading_lo': 0.8,       # 直线段增益（|h|<hk1_deg，低增益防摆）
    'kp_heading_mid': 0.8,      # 小弯段增益（hk1~hk2）
    'kp_heading_hi': 0.75,      # 大弯段增益（>hk3）【0.9→0.85：再消一小点内切】
    'hk1_deg': 5.0,             # 直线/小弯过渡起点
    'hk2_deg': 10.0,            # 小弯/中弯过渡
    'hk3_deg': 18.0,            # 大弯起始，之间线性插值平滑切换
    'kp_center': 0.018,         # rad/s / px(近场横向误差)【0.018→0.020：回收更果断】
    'kd_heading': 0.10,         # 朝向微分阻尼（s，0.25s 长窗口差分）
    'kd_center': 0.005,         # 横向速率阻尼（s，杀横向极限环）
    'heading_spike_max_deg': 5.0,  # heading 单帧跳变抑制阈值
    'ema_h': 0.35,              # heading EMA 平滑系数
    'ema_c': 0.55,              # center EMA 平滑系数
    'wz_slew': 3.0,             # wz 输出斜率限制 (rad/s^2)，防硬拉瞬间反向
    'wz_max': 0.55,             # 角速度限幅（大弯够用）
    'speed_h_deg': 12.0,        # |h| 超过此值开始降速（弯道）
    'speed_k_deg': 0.004,       # |h| 每超过阈值 1° 的降速量 (m/s)
    'kp_lat': 0.0,              # 侧移 linear.y 增益（0=禁用）
    'heading_bias_deg': 0.0,    # 朝向系统偏差补偿
    'deadband_center_px': 3.0,
    'deadband_heading_deg': 1.2,
}

# 控制误差平滑系数
EMA_ALPHA = 0.55        # (已弃用，改用 TUNE['ema_h']/['ema_c']，保留仅为兼容)
...
class SharedState:
    ...
    def status(self):
        return {
            ...
            'record_active': bool(self.rec_recording),
            'record_path': self.rec_path,
            'ctl': dict(getattr(self, 'ctl', {})),
            'tune': dict(TUNE),
        }
...
def compute_pid_ipm(heading_deg, center_px, state):
    ...
    - heading_deg 实为前瞻点方位角（含横向+朝向），天然自阻尼，转向平滑
    - heading 尖峰抑制：单帧跳变 > threshold 视为坏帧，沿用旧滤波值
    - EMA 平滑（heading 更平滑）
    - 0.25s 长窗口微分（heading + center 各一组），避免放大单帧抖动
    - 分段增益：|h| 小 -> kp_lo；|h| 大 -> kp_hi；之间线性插值平滑过渡
    - 弯道降速：|h| 超过 speed_h_deg 后线性降速，给足转向抓地力
    """
    heading_deg -= TUNE['heading_bias_deg']

    # heading 尖峰抑制（帧间跳变过大视为坏帧）
    ...
    # 非线性分段增益（直线低增益防摆，弯道高增益强打，三档平滑过渡）
    ah = abs(h)
    if ah <= TUNE['hk1_deg']:
...
def control_timer(_event):
    with state.lock:
        ...
        # 停止线蠕动（启动后不依赖停止线持续可见：线贴地时易出视野底部，
        # 若仅以 stop_line_detected 守卫，线一消失蠕动就中断、恢复全速，
        # 导致车绕过一圈再次撞线。改为 creep_started 驱动，走满 CREEP_DISTANCE 即停）
        ...
        state.ctl['gentle'] = bool(is_gentle)
        state.ctl['wz_final'] = float(vel.angular.z)
...
<style>body{font:16px sans-serif;background:#0f172a;color:#f8fafc;margin:20px}main{max-width:1100px;margin:auto}section{background:#1e293b;padding:16px;margin:12px 0;border-radius:10px}img{width:32%;background:#111;margin:.33%;border-radius:6px}.start{background:#1677ff;color:white}.stop{background:#d00;color:white}.rec_on{background:#0a9d58;color:white}.rec_off{background:#334155;color:white}button{padding:10px 18px;border:0;border-radius:6px;margin-right:8px;font-size:15px;cursor:pointer}pre{font-size:15px;background:#0f172a;padding:10px;border-radius:6px;color:#38bdf8;white-space:pre-wrap}</style>
<main><h2>纯巡线控制台 (IPM 鸟瞰版, Port 5007)</h2>
<section><b>确认跑道清空后手动启动巡线。</b>
...
<p>目标速度：
<button onclick="post('/api/set_speed?speed=0.18')">0.18</button>
<button onclick="post('/api/set_speed?speed=0.24')">0.24</button>
<button onclick="post('/api/set_speed?speed=0.36')">0.36 m/s</button></p>
<p><button id="rec_btn" class="rec_off" onclick="toggleRec()">开始记录数据</button> <span id="rec_info"></span></p>
<pre id="status">加载状态中...</pre></section>
<section><h3>原图叠加 · 鸟瞰二值 · 鸟瞰彩色</h3>
...
async function toggleRec(){
    let url = recording ? '/api/record_stop' : '/api/record_start';
    try {
        let r = await fetch(url,{method:'POST'});
        let d = await r.json();
        if(d.ok){
            recording = !recording;
            document.getElementById('rec_btn').textContent = recording ? '停止记录数据' : '开始记录数据';
            document.getElementById('rec_btn').className = recording ? 'rec_on' : 'rec_off';
            if(d.path){ document.getElementById('rec_info').textContent = '已保存: ' + d.path; }
        } else { alert("⚠️ 记录失败: " + (d.error||'')); }
    } catch(e) { alert("⚠️ 请求失败: " + e); }
}
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            while not rospy.is_shutdown():
                # 锁内只做快照，编码与网络写放锁外（防止慢客户端写阻塞卡死全节点）
                with state.lock:
    ...
    def do_POST(self):
        ...
        with state.lock:
            if path == '/api/start':
                ...
                publish_stop()
                self.reply({'ok': True})
            elif path == '/api/record_start':
                path, err = record_start()
                if err:
                    self.reply({'ok': False, 'error': err})
                else:
                    self.reply({'ok': True, 'path': path})
            elif path == '/api/record_stop':
                path, err = record_stop()
                if err:
                    self.reply({'ok': False, 'error': err})
                else:
                    self.reply({'ok': True, 'path': path})
...
RECORD_HEADER = [
    ...
    'wheel_vx', 'wheel_vy', 'wheel_wz', 'imu_wz',
    'wheel_px', 'wheel_py', 'wheel_pyaw',
    'lf_mode', 'lf_error_deg', 'lf_heading_error_deg', 'lf_center_error_px',
    'lf_center_count', 'lf_kanbujian', 'lf_vision_valid',
    'lf_lane_tracks', 'lf_pair_valid', 'lf_far_confidence',
    'lf_stop_line_detected', 'lf_image_age',
...
def imu_cb(data):
    with state.lock:
        state.imu_wz = data.angular_velocity.z
...
def record_start():
    with state.lock:
        if state.rec_recording:
            return None, '已在记录中'
        rec_dir = os.path.expanduser('~/Xunxian_standalone/records')
        os.makedirs(rec_dir, exist_ok=True)
        timestamp = time.strftime('%Y%m%d_%H%M%S')
        path = os.path.join(rec_dir, f'diag_record_{timestamp}.csv')
        try:
            f = open(path, 'w', newline='')
        except Exception as e:
            return None, f'无法创建记录文件: {e}'
        state.rec_file = f
        state.rec_writer = csv.writer(f)
        state.rec_writer.writerow(RECORD_HEADER)
        state.rec_path = path
        state.rec_recording = True
        rospy.loginfo('开始记录数据: %s', path)
        return path, None


def record_stop():
    with state.lock:
        if not state.rec_recording:
            return None, '未在记录中'
        path = state.rec_path
        try:
            state.rec_file.flush()
            state.rec_file.close()
        except Exception:
            pass
        state.rec_file = None
        state.rec_writer = None
        state.rec_recording = False
        rospy.loginfo('停止记录: %s', path)
        return path, None


def record_tick(_event):
    if not state.rec_recording or state.rec_writer is None:
        return

    # 1. 锁内只做数据快照（微秒级完成），防止占用锁卡死图像/控制/Web线程
    with state.lock:
        if not state.rec_recording or state.rec_writer is None:
            return
        f_writer = state.rec_writer
        f_file = state.rec_file
        now = rospy.Time.now().to_sec()
        wall_t = time.time()
        vx, vy, wz = state.command_linear_x, state.command_linear_y, state.command_angular_z
        imu_wz = state.imu_wz
        ox, oy, oyaw = state.odom_x, state.odom_y, state.odom_yaw
        mode = state.mode
        h_deg = state.heading_error_deg
        c_px = state.center_error_px
        n_centers = len(state.centers)
        kanbu = int(state.kanbujian)
        vis_valid = int(state.vision_valid)
        tracks = int(state.track_left is not None) + int(state.track_right is not None)
        stop_line = int(state.stop_line_detected)
        img_age = max(0.0, time.time() - state.last_image_time)

    # 2. 锁外进行文件写入与磁盘 flush（写磁盘延迟不影响全局锁）
    try:
        if f_writer is not None:
            f_writer.writerow([
                ...
                0.0, 0.0, 0.0, imu_wz,
                ox, oy, oyaw,
                mode, h_deg, h_deg, c_px,
                n_centers, kanbu, vis_valid,
                tracks, int(not kanbu), 0,
                stop_line, img_age,
                ox, oy, oyaw,
            ])
        ...
        rospy.logwarn_throttle(2, '记录写入异常: %s', e)
...
def main():
    ...
    rospy.Subscriber('/imu/physically_inverted', Imu, imu_cb, queue_size=1)
    rospy.Timer(rospy.Duration(0.05), control_timer)  # 20Hz
    rospy.Timer(rospy.Duration(0.1), record_tick)     # 10Hz 数据记录
