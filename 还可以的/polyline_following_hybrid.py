#!/usr/bin/env python3
"""
polyline_following_hybrid.py

视觉 + 雷达 混合引导：
  视觉段（polyline_following_ipm_0804.py）：
    DISARMED -> F_ALIGN(对面中线平移对准) -> ADVANCE(前进50cm 尝试巡线稳定)
            -> LINE_FOLLOW(巡线PID) -> TURN_RIGHT(前进右转47°) -> SEARCH_RIGHT(原地右转搜索) -> LINE_FOLLOW(第2段斜车道)
  雷达段（polyline_following_lidar.py）：
    RADAR_PREALIGN -> RADAR_APPROACH -> RADAR_TURN -> RADAR_PARALLEL -> RADAR_DONE(前方墙停车)

接管触发（双门，不用计数器）：
  1) 状态门 y_turn_done：仅在 TURN_RIGHT / SEARCH_RIGHT 成功转回 LINE_FOLLOW 时置 True，任务启动/复位清零；
  2) 感知门 radar_ready：scan 新鲜 且 left_wall 检测到且到墙垂直距离 perp ∈ [0.25, 0.40]
     连续 RADAR_HANDOFF_FRAMES 帧（即雷达测到小车垂直距离墙壁约 40 时才接管）。
  两门同时满足 -> publish_stop -> RADAR_PREALIGN（自动接管，无需按键）。

雷达 -> 视觉兜底（radar_fallback）：
  雷达段任一处 scan 超时 / APPROACH·TURN·PARALLEL 左墙连续丢失 RADAR_WALL_LOST_FRAMES 帧 /
  PREALIGN 超时且无左墙 -> 复位视觉感知状态 -> 切回 LINE_FOLLOW
  （视觉自身丢失->TURN_RIGHT 兜底；停止线检测已启用，可走到停止线停车）。
  y_turn_done 保持 True，RADAR_RETRY_COOLDOWN 秒后可重新接管（防止抖动反复横跳）。

启动：rosrun polyline_following_hybrid.py（需相机 /usb_cam/image_raw、/odom、雷达 /scan 均在线；
      雷达不可用时视觉全程兜底）。网页控制台端口 5010 启动/停止/复位。
"""
import collections
import json
import math
...
RIGHT_TRUST_NX_MIN = 340.0
RIGHT_TRUST_SPAN_MIN = 60.0
RIGHT_TRUST_FRAMES = 5
RIGHT_TRUST_JITTER_PX = 25.0

SEARCH_ROTATE_WZ = -0.25
SEARCH_ACCUM_LIMIT_DEG = 70.0
SEARCH_TIMEOUT = 15.0
...
# ==================== 雷达段参数（polyline_following_lidar.py） ====================
KEEP_WALL_DIST = 0.26      # 贴墙安全距离 m（平行段保持目标）
TURN_TRIGGER_DIST = 0.36   # 左墙距离 <= 此值触发转弯
...
SKIP_PREALIGN = True       # 临时跳过原地对准：45°为猜测值，先让纯视觉自然对准看真实车-墙角
...
# ==================== 混合接管 / 兜底参数 ====================
RADAR_HANDOFF_PERP_MIN = 0.25    # 接管感知门：到墙垂直距离下限(m)
RADAR_HANDOFF_PERP_MAX = 0.40    # 接管感知门：到墙垂直距离上限(m)，雷达测到<=40 才允许接管
RADAR_HANDOFF_FRAMES = 5      # 感知门连续满足帧数(20Hz)
RADAR_WALL_LOST_FRAMES = 12   # APPROACH/TURN/PARALLEL 左墙连续丢失帧数 -> 回退视觉
RADAR_RETRY_COOLDOWN = 3.0    # 回退视觉后重新接管的冷却时间(s)，防反复横跳

RADAR_MODES = ('RADAR_PREALIGN', 'RADAR_APPROACH', 'RADAR_TURN', 'RADAR_PARALLEL', 'RADAR_DONE')
VIS_MODES = ('F_ALIGN', 'ADVANCE', 'LINE_FOLLOW', 'TURN_RIGHT', 'SEARCH_RIGHT')
...
class SharedState:
    def __init__(self):
        ...
        # 混合：雷达接管/兜底
        self.y_turn_done = False        # 状态门：已完成一次右转
        self.radar_ready_frames = 0     # 感知门连续满足帧数
        self.radar_wall_lost_frames = 0 # 雷达段左墙丢失计数
        self.radar_fallback_time = 0.0  # 上次回退视觉的时刻(重新接管冷却)
    ...
    def status(self):
        return {
            ...
            'y_turn_done': bool(self.y_turn_done),
            'radar_available': bool(radar_available()),
            'radar_ready_frames': int(self.radar_ready_frames),
            'radar_wall_lost_frames': int(self.radar_wall_lost_frames),
            'radar_d': round(left_wall['d_min'], 3) if left_wall is not None else None,
            'radar_perp': round(left_wall['perp'], 3) if left_wall is not None else None,
            'radar_line': round(left_wall['line_angle'], 1) if left_wall is not None else None,
            'radar_front': round(front_wall['d_min'], 3) if front_wall is not None else None,
        }
...
def radar_available():
    return (scan_time > 0.0) and (time.time() - scan_time <= SCAN_TIMEOUT) and (left_wall is not None)
...
def image_cb(msg):
    ...
    try:
        ...
        # ADVANCE 前 30cm 与转弯后的巡线(TURN_RIGHT/SEARCH_RIGHT)以及雷达段一律用紧 ROI。
...
# ==================== 混合接管 / 兜底 ====================
def try_radar_handoff(now):
    """双门接管判定：状态门 y_turn_done(已过Y型右转) + 感知门(scan新鲜&左墙在接近位形)。
    在 state.lock 内调用。"""
    if now - state.radar_fallback_time < RADAR_RETRY_COOLDOWN:
        return False
    scan_ok = (time.time() - scan_time) <= SCAN_TIMEOUT
    lw = left_wall
    gate = scan_ok and (lw is not None) and (RADAR_HANDOFF_PERP_MIN <= lw['perp'] <= RADAR_HANDOFF_PERP_MAX)
    if gate:
        state.radar_ready_frames += 1
    else:
        state.radar_ready_frames = 0
    return state.radar_ready_frames >= RADAR_HANDOFF_FRAMES


def radar_fallback(reason):
    """雷达异常 -> 复位视觉感知状态 -> 切回 LINE_FOLLOW。在 state.lock 内调用。"""
    rospy.logwarn('雷达异常: %s', reason)
    state.message = '雷达异常: %s' % reason
    ...
    state.heading_samples.clear()
    state.center_samples.clear()
    state.right_good_frames = 0
    ...
    state.roi_wide_remaining = 0
    state.lane_bias_px = 0.0
    state.radar_ready_frames = 0
    state.radar_wall_lost_frames = 0
    state.radar_fallback_time = time.time()
    state.stop_line_enabled = True
    state.stop_line_enable_time = time.time()
    state.mode = 'LINE_FOLLOW'
    publish_stop()


def radar_control(now, lw, fw):
    """雷达段控制（RADAR_PREALIGN -> APPROACH -> TURN -> PARALLEL）。在 state.lock 内调用。"""
    global int_err, last_par_t, prealign_t0
    if time.time() - scan_time > SCAN_TIMEOUT:
        radar_fallback('scan超时，回退视觉巡线')
        return

    if state.mode in ('RADAR_APPROACH', 'RADAR_TURN', 'RADAR_PARALLEL'):
        if lw is None:
            state.radar_wall_lost_frames += 1
        else:
            state.radar_wall_lost_frames = 0
        if state.radar_wall_lost_frames >= RADAR_WALL_LOST_FRAMES:
            radar_fallback('左墙连续丢失，回退视觉巡线')
            return
    ...
    if mode == 'RADAR_PREALIGN':
        if SKIP_PREALIGN:
            # 临时跳过原地对准：直接进入 APPROACH，按视觉留下的姿态贴墙接近
            state.mode = 'RADAR_APPROACH'
            cmd.linear.x = FORWARD_SPEED
            cmd.angular.z = 0.0
            src = '->RADAR_APPROACH(PREALIGN_SKIPPED)'
        elif lw is None:
            if now - prealign_t0 > ALIGN_TIMEOUT:
                radar_fallback('PREALIGN超时且无左墙，回退视觉')
                return
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
        ...
        if fw is not None and fw['d_min'] < FRONT_SLOW_DIST:
            if fw['d_min'] < FRONT_STOP_DIST:
                state.mode = 'RADAR_DONE'
                state.message = '前方墙 %.3f m，停车完成' % fw['d_min']
...
def control_timer(_event):
    ...
    with state.lock:
        ...
        if mode in ('DISARMED', 'STOPPED'):
            publish_stop()
            return

        # 全局安全：任意方向过近急停（有 scan 数据时）
        if all_min_d < MIN_SAFE_DIST and scan_time > 0.0:
            state.mode = 'ESTOP'
            state.message = '安全触发 too_close_%.3f' % all_min_d
            rospy.logerr('安全触发: too_close_%.3f', all_min_d)
            publish_stop()
            return

        if mode in ('ESTOP', 'FAULT', 'RADAR_DONE'):
            publish_stop()
            return

        if mode in RADAR_MODES:
            lw = dict(left_wall) if left_wall else None
            fw = dict(front_wall) if front_wall else None
            radar_control(now, lw, fw)
            return
        ...
        # 雷达接管（仅在第2段 LINE_FOLLOW：已完成右转 且 感知门满足）
        if mode == 'LINE_FOLLOW' and state.y_turn_done:
            if try_radar_handoff(now):
                state.mode = 'RADAR_PREALIGN'
                state.radar_wall_lost_frames = 0
                prealign_t0 = now
                state.message = '雷达接管：贴墙引导'
                rospy.loginfo('>>> 雷达接管：贴墙引导')
                publish_stop()
                return
        ...
        if mode == 'F_ALIGN':
            ...
            # 前进50cm，同时检查巡线是否稳定（必须双线同时检测到才切换）
...
<title>polyline_following_hybrid 混合节点</title>
<style>
  ...
  .status-SEARCH_RIGHT { background: #c026d3; }
  .status-RADAR_PREALIGN, .status-RADAR_APPROACH { background: #16a34a; }
  .status-RADAR_TURN, .status-RADAR_PARALLEL { background: #0d9488; }
  .status-RADAR_DONE { background: #22c55e; }
...
<body>
<main>
  <h2>polyline_following_hybrid 混合节点</h2>
  <section>
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <span><strong>当前状态：</strong> <span id="mode_badge" class="status-badge">DISARMED</span></span>
      <span style="color:#94a3b8; font-size:14px;">视觉巡线 ➔ Y型右转 ➔ 雷达贴墙右转 ➔ 前方墙停车</span>
    </div>
    <div class="btn-group">
    ...
    <div class="card-grid">
      ...
      <div class="card"><div class="card-title">已过Y型右转</div><div class="card-value" id="val_yturn">否</div></div>
      <div class="card"><div class="card-title">雷达可用</div><div class="card-value" id="val_radar">-</div></div>
      <div class="card"><div class="card-title">左墙距离</div><div class="card-value" id="val_ld">-</div></div>
      <div class="card"><div class="card-title">垂直距离</div><div class="card-value" id="val_perp">-</div></div>
      <div class="card"><div class="card-title">左墙线角</div><div class="card-value" id="val_lang">-</div></div>
      <div class="card"><div class="card-title">接管就绪帧</div><div class="card-value" id="val_ready">0 / 5</div></div>
...
setInterval(async () => {
  try {
    ...
    document.getElementById('val_yturn').textContent = data.y_turn_done ? '是' : '否';
    document.getElementById('val_radar').textContent = data.radar_available ? '是' : '否';
    document.getElementById('val_ld').textContent = data.radar_d !== null ? data.radar_d.toFixed(3) : '-';
    document.getElementById('val_perp').textContent = data.radar_perp !== null ? data.radar_perp.toFixed(3) : '-';
    document.getElementById('val_lang').textContent = data.radar_line !== null ? data.radar_line.toFixed(1) + '°' : '-';
    document.getElementById('val_ready').textContent = data.radar_ready_frames + ' / 5';
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with state.lock:
            if path == '/api/start':
                if state.mode in VIS_MODES or state.mode in RADAR_MODES:
                ...
                state.y_turn_done = False
                state.radar_ready_frames = 0
                state.radar_wall_lost_frames = 0
                state.radar_fallback_time = 0.0
                state.mode = 'F_ALIGN'
    ...
    def server_bind(self):
        try:
            ...
            if getattr(e, 'errno', None) == 98:
                ...
                os.system(f"fuser -k {PORT}/tcp 2>/dev/null || pkill -9 -f polyline_following_hybrid.py 2>/dev/null")
                time.sleep(1)
                super().server_bind()
            else:
                raise
...
if __name__ == '__main__':
    rospy.init_node('polyline_following_hybrid', anonymous=False)
    init_ipm()
    load_hsv()
    ...
    rospy.loginfo('polyline_following_hybrid 已启动 (网页控制端口 http://0.0.0.0:%d)', PORT)
